...
css/
  style.css         スタイルシート
  softkbd.css       仮想キーボードのスタイル
core/               共有エンジン（View 横断で再利用する中核）
  index.js          コアの公開 API 窓口（FM7 / D77Disk / キーボード定数 等を再エクスポート）
  fm7.js            システム全体の統合クラス（メモリマップ、I/O、スケジューラ連携）
  cpu6809.js        MC6809 CPU エミュレーション
  display.js        画面描画（VRAM、ALU、ライン描画、パレット、320×200/640×200/640×400/262K色）
  ...
  softkbd.js        仮想キーボード（タッチ端末向け。スマホ縦向き専用レイアウト、画面直下ドック）
...
FM音源部（core/opn.js）には別途の利用条件が適用されます。詳細は [LICENSE.md](LICENSE.md) を参照してください。
