...
2. **Machine** を機種に合わせて選択（FM-7 / FM77AV / FM77AV20 / FM77AV20EX / FM77AV40 / FM77AV40EX/SX）。
3. **Boot Mode** を **BASIC** に設定。
4. **Power** を ON。`OK` プロンプト（F-BASIC READY）が表示されるまで待つ。
...
4. 2 本組や「ローダー＋本体」構成のテープは、ローダーが起動した後に本体テープへ入れ替えて読み込み操作を行う（実機と同じ流れ）。

> ローダーが本体を読みに行く形式のテープでは、ローダー部と本体部の両方が 1 本の WAV に含まれていれば、そのまま通して読み込めます。
...
`SAVE` / `SAVEM` 実行後、エミュレータは書き込み内容を**録音**しています。
...
1. **Library** の「取り込み」… ではなく、**録音→.t77** で一旦ダウンロード → そのファイルを
   ...
   （SAVE 録音から直接 Library に追加する操作は今後の改善候補）
...
実機 FM-7 では `SAVE` 実行時、BIOS がカセットの WRITE データ線（`$FD00` bit 0）を
1200Hz / 2400Hz の波形でトグルします。WebM7 はこのトグルを CPU サイクル単位で
追跡し、T77 形式のパルス幅列としてバッファに**録音**しています。
**録音→.t77** ボタンを押した時点で、バッファ内容をリトルエンディアンの T77 として
...
5. `NEW` ＋ RETURN（または **Reset**）でメモリを消去。
...
- WebM7 の 録音→.t77 はリトルエンディアン書き出し。LOAD 側は BE/LE を自動検出するため、
  どちらでも読めます。
- ファイル先頭にマジックヘッダ + null 1 バイト = 17 バイトのヘッダ、その後にパルス列が続きます。

![図: T77 ファイルの構造](images/t77_06_format.svg)

17 バイトのヘッダの後に 16bit のパルスエントリが並び、`0x0000` は無音（ギャップ）を表します。
...
| 保存した T77 が他環境で開けない | WebM7 はリトルエンディアンで T77 を書き出します。多くのツールは自動検出するため通常は問題なし。 |
| 同じ SAVE 内容と新しい SAVE が混ざる | SAVE 前に **録音クリア** で録音バッファをクリア。 |
| `LOAD` 中に進捗が止まる | エミュレータがテープ読み込み時に自動高速化（CMT turbo）するので、通常は数秒で完了。停止する場合は **Reset** → 再ロード。 |
...
- 途中で止まる … 本体プログラムが別フォーマット（生データ）の可能性。WebM7 は自動対応しますが、録音レベル不足だと途中で途切れることがあります。
...
- **複数機種・複数 ROM 版での互換性**は基本的に問題ありませんが、機種固有の
  `SAVE` BIOS の挙動差がある場合は別途調査します。
